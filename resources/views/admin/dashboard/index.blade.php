...
<div class="container mx-auto py-10 px-6 font-open-sans animate-fade-in">
    ...
    <h1 class="mb-8 text-5xl font-extrabold text-center text-primary-dark font-headings animate-fade-in-up leading-tight">Dashboard Estadístico – Administrador</h1>
<div class="flex justify-end gap-4 mb-8">
    <a href="{{ route('admin.dashboard.export.excel', request()->all()) }}" class="bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 transition">Exportar a Excel</a>
    <a href="{{ route('admin.dashboard.export.pdf', request()->all()) }}" class="bg-red-600 text-white px-5 py-2 rounded-lg hover:bg-red-700 transition">Exportar a PDF</a>
</div>

    {{-- Formulario de filtros --}}
    <form method="GET" action="{{ route('admin.dashboard') }}" class="mb-10 p-7 bg-white rounded-xl shadow-2xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 items-end animate-slide-in-down border border-gray-200">
        <div>
            <label for="year_from" class="block text-gray-corporate text-sm font-semibold mb-2">Cohorte desde</label>
            <input type="number" name="year_from" id="year_from" value="{{ request('year_from') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-secondary focus:ring-blue-secondary transition duration-200 ease-in-out px-4 py-2.5 text-text-main placeholder-text-light" placeholder="Ej: 2020" />
        </div>

        <div>
            <label for="year_to" class="block text-gray-corporate text-sm font-semibold mb-2">Cohorte hasta</label>
            <input type="number" name="year_to" id="year_to" value="{{ request('year_to') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-secondary focus:ring-blue-secondary transition duration-200 ease-in-out px-4 py-2.5 text-text-main placeholder-text-light" placeholder="Ej: 2024" />
        </div>

        <div>
            <label for="sector" class="block text-gray-corporate text-sm font-semibold mb-2">Sector</label>
            <select name="sector" id="sector" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-secondary focus:ring-blue-secondary transition duration-200 ease-in-out px-4 py-2.5 text-text-main">
        ...
        <div>
            ...
            <select name="gender" id="gender" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-secondary focus:ring-blue-secondary transition duration-200 ease-in-out px-4 py-2.5 text-text-main">
                <option value="">Todos</option>
        ...
        <div>
            ...
            <input type="text" name="position" id="position" value="{{ request('position') }}" placeholder="Ej. Desarrollador" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-secondary focus:ring-blue-secondary transition duration-200 ease-in-out px-4 py-2.5 text-text-main placeholder-text-light" />
        </div>

        <div>
            <button type="submit" class="bg-primary text-white font-bold py-2.5 px-5 rounded-lg hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary-light focus:ring-opacity-75 w-full transition duration-300 ease-in-out transform hover:scale-105 shadow-md">
                Filtrar Resultados
            </button>
        </div>
    </form>
    ...
    @foreach($unreadNotifications as $notification)
        <div class="bg-blue-200 border-l-4 border-blue-secondary text-blue-800 p-5 shadow-lg mb-8 animate-fade-in-down rounded-lg flex items-center space-x-4" role="alert">
            <div class="flex-shrink-0">
                <svg class="fill-current h-7 w-7 text-blue-secondary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        <x-dashboard.card title="Empleabilidad por Cohorte" icon="trending-up" class="bg-primary-dark text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
                <canvas id="chartCohortes"></canvas>
            </div>
        </x-dashboard.card>

        <x-dashboard.card title="Distribución por Sector Laboral" icon="briefcase" class="bg-secondary text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
                <canvas id="chartSectores"></canvas>
            </div>
        </x-dashboard.card>

        <x-dashboard.card title="Distribución por País" icon="globe" class="bg-blue-institutional text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
                <canvas id="chartPaises"></canvas>
            </div>
        </x-dashboard.card>

        <x-dashboard.card title="Top Habilidades Técnicas" icon="cpu" class="bg-accent text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
                <canvas id="chartSkills"></canvas>
            </div>
        </x-dashboard.card>

        <x-dashboard.card title="Distribución por Género" icon="users" class="bg-gray-corporate text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
                <canvas id="chartGenero"></canvas>
            </div>
        </x-dashboard.card>

        <x-dashboard.card title="Top Ciudades de Trabajo" icon="map-pin" class="bg-logo-primary text-white shadow-3xl animate-pop-in">
            <div class="relative h-64"> {{-- Altura fija para gráficos --}}
    ...
    const baseChartOptions = {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                borderWidth: 1,
                cornerRadius: 5,
                titleFont: { family: 'Open Sans', size: 14, weight: 'bold' },
    ...
    new Chart(document.getElementById('chartGenero'), {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#6B7280','#EF4444','#10B981','#BFD7ED'], // gray, red, green, logo-blue-200
